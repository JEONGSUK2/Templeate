...
function View() {
    ...
    const viewCnt = async(borad, view) => {
        const viewRef = doc(getFirestore(),borad,view);
    ...
    if (isModal) {
        return (
            isModal && view && <Modal error="해당문서가 존재하지 않습니다." onClose={() => { setIsModal(false); }} />
        )
    }
    if (!post) {
        return (
